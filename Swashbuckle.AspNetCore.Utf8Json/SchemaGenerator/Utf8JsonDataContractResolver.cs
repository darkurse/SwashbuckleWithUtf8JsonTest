...
namespace Swashbuckle.AspNetCore.Utf8Json
{
    public class Utf8JsonDataContractResolver : ISerializerDataContractResolver
    {
        private readonly IJsonFormatterResolver _formatterResolver;

        public Utf8JsonDataContractResolver(IJsonFormatterResolver formatterResolver)
        {
        ...
        {
            return Encoding.UTF8.GetString(JsonSerializer.Serialize(value, _formatterResolver));
        }
            ...
            var applicableProperties = publicProperties
                .Where(property =>
                {
                    return
                        ...
                        !(property.HasAttribute<IgnoreDataMemberAttribute>());
                    // && !(_serializerOptions.IgnoreReadOnlyProperties && !property.IsPubliclyWritable()); -- Impossible to know with Utf8Json
                })
        ...
        private string ConvertName(string propertyName)
        {
            Type type = _formatterResolver.GetType();
            FieldInfo info = type.GetField("resolvers", BindingFlags.NonPublic | BindingFlags.Static);

            IJsonFormatterResolver[] values = (IJsonFormatterResolver[])info.GetValue(null);
            foreach (var value in values)
            {
                string resolverName = value.GetType().Name.ToLowerInvariant();

                if (resolverName.Contains("camelcase"))
                    return JsonPropertyNameMutator.ToCamelCase(propertyName);
                else if (resolverName.Contains("snakecase"))
                    return JsonPropertyNameMutator.ToSnakeCase(propertyName);
            }

            return JsonPropertyNameMutator.Original(propertyName);
        }
